```{r}
pacman::p_load(sf, raster, spatstat, tmap, tidyverse, RColorBrewer, spdep, sfdep, ggplot2, jsonlite, olsrr, corrplot, ggpubr, GWmodel, gtsummary)
```
...
```
```{r}
resale_5yr_sf <- read_rds("data/resale_5yr_sf.rds")
resale_sf <- read_rds("data/resale_sf.rds")
```
